...
it("Should not reject when the batch cannot be serialized", async () => {
  const errorSpy = jest.spyOn(console, "error").mockImplementation();
  ...
  // Carried on the event context, so it reaches HumioZink's serializer intact
  // rather than being rendered into the message up front.
  const event = new LogEvent(
    LogEventLevel.Information,
    new LogEventMessage("Hello!", [], LogEventMessageDefaultSerializer),
    new Map([["Payload", unserializable]]),
  );
  ...
  // batchSizeLimit 0 sends on the first log(), and nothing awaits that send —
  // a throw out of the serializer used to escape as an unhandled rejection.
  expect(() => humioZink.log(event)).not.toThrow();

  await Promise.resolve();

  expect(fetchSpy).not.toHaveBeenCalled();
  expect(errorSpy).toHaveBeenCalledWith(
    "[Zerilog/HumioZink] Failed to send batch",
    expect.any(Error),
  );
